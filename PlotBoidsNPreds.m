function [figHandler, boidHandler, predHandler] = PlotBoidsNPreds(...
    boidPositions,boidVelocities,predPositions,predVelocities,maxPositions)
figHandler = figure('Position',[100 100 800 600])
boidHandler = quiver3(boidPositions(:,1),boidPositions(:,2),...
    boidPositions(:,3),boidVelocities(:,1),boidVelocities(:,2), ...
    boidVelocities(:,3),'bo','Autoscale','off');
hold on
predHandler = quiver3(predPositions(:,1),predPositions(:,2),...
    predPositions(:,3),predVelocities(:,1),predVelocities(:,2), ...
    predVelocities(:,3),'ro','Autoscale','off');
